...
| 8–12 | Hearts / Clubs / Stars / Rings / Bolts | `symbolGrid()` of one path from `SHAPES` (see below)                                                                                                                            | nothing         |
| 13   | Diagonal split                         | label's bottom-right half in the pattern color, corner to corner; the split line is a quadratic bowed by `SPLIT_CURVE` toward the pattern side, so it reads concave; drawn flat | nothing         |
| 14   | Vee                                    | a single bold V (`VEE_SIZE` stroke), legs running from the top corners to a point at the bottom center, drooped per point so it wraps the can                                   | nothing         |
| 15   | Bullseye                               | concentric rings (`TARGET_SIZE` stroke, `TARGET_GAP` spacing) from the label center outward, eye pinned to the center, drooped per point                                        | nothing         |
| 16   | Big X                                  | two bold diagonals corner to corner (`EX_SIZE` stroke), drooped per point                                                                                                       | nothing         |
| 17   | Wavy split                             | label's bottom half in the pattern color; the top boundary is a small three-peak wave, drooped per point                                                                        | nothing         |
